...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_step_speed.csv'))['step speed'].std())
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        if dir_name[:8] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_step_speed.csv'))
        #data_['arm'][data_['arm'] > 1] = 1 / data_['arm'][data_['arm'] > 1]

        original_data = data_
        original_variable = original_data['step speed']

        #normalization
        #data_ = remove_outliers_3(data_, 'step speed')
        data_['step speed'] = (data_['step speed'] - data_['step speed'].min()) / (data_['step speed'].max() - data_['step speed'].min())
        ...
        data['stride_rolling'] = data['step speed'].rolling(window=4).mean()
        data['converttime'] = data['time'].apply(lambda x:convert_video(x))

        original_data['stride_rolling'] = original_data['step speed'].rolling(window=4).mean()
...
plt.plot(after_1['rate_rolling'], after_1['trend'], color='#2E5DA5', label="low level of cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#2E5DA5', label="moderate level of cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#2E5DA5', label="high level of cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#2E5DA5', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#2E5DA5', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#2E5DA5', alpha=0.2)
...
plt.xlabel('pupil dilation', fontsize = 20,fontname='Arial')
plt.ylabel('step speed', fontsize = 20,fontname='Arial')
plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=2, frameon=False,columnspacing=7.5,fontsize=14)
...
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_step_speed_120.png')
...
# stride_sort_2 = stride_sort_2.dropna()
# stride_sort_3 = stride_sort_3.dropna()
stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'step_speed_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'step_speed_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'step_speed_merge3.csv'),index=False)
