...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_gait_cycle.csv'))['gait cycle'].std())
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        if dir_name[:-3] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_gait_cycle.csv'))
        #data_['arm'][data_['arm'] > 1] = 1 / data_['arm'][data_['arm'] > 1]

        original_data = data_
        original_variable = original_data['gait cycle']

        #normalization
        data_ = remove_outliers_3(data_, 'gait cycle')
        data_['gait cycle'] = (data_['gait cycle'] - data_['gait cycle'].min()) / (data_['gait cycle'].max() - data_['gait cycle'].min())
        ...
        # data
        data['stride_rolling'] = data['gait cycle'].rolling(window=5).mean()
        data['converttime'] = data['time'].apply(lambda x:convert_video(x))

        original_data['stride_rolling'] = original_data['gait cycle'].rolling(window=5).mean()
        ...
        if dir_name[-1] == '1' :
            ...
            original_merge = original_merge[(original_merge['rate_rolling'] >= -0.005833185017907219) & (original_merge['rate_rolling'] <= 0.044140670424346704)]
            if len(original_merge) <= 1: continue
            correlation_coefficient, p_value = spearmanr(original_merge['rate_rolling'], original_merge['stride_rolling'])
            ...
            x = x + 1
            if x == 1 or x == 32 or x == 8 or x == 45 or x==15:
                continue
            ...
            original_merge = original_merge[(original_merge['rate_rolling'] >= 0.044140670424346704) & (original_merge['rate_rolling'] <= 0.11199983686890083)]
            if len(original_merge) <= 1:
                print(dir_name)
                continue
            correlation_coefficient, p_value = spearmanr(original_merge['rate_rolling'], original_merge['stride_rolling'])
...
plt.plot(after_1['rate_rolling'], after_1['trend'], color='#EF82B2', label="low cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#EF82B2', label="moderate cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#EF82B2', label="high cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#EF82B2', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#EF82B2', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#EF82B2', alpha=0.2)

plt.text(-0.075, y_axis_max - 0.015, 'r=0.040, p=0.003*', fontsize=14)
plt.text(0, y_axis_max - 0.015, 'r=-0.061, p=0.022*', fontsize=14)
plt.text(0.06, y_axis_max - 0.015, 'r=0.051, p=0.044*', fontsize=14)

plt.text(-0.075, y_axis_max - 0.022, equation1, fontsize=14)
plt.text(0, y_axis_max - 0.022, equation2, fontsize=14)
plt.text(0.06, y_axis_max - 0.022, equation3, fontsize=14)


plt.xlabel('Pupil dilation ratio', fontsize = 20,fontname='Arial')
plt.ylabel('Gait cycle', fontsize = 20,fontname='Arial')
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_gait_cycle_60.png')
...
stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'gait_cycle_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'gait_cycle_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'gait_cycle_merge3.csv'),index=False)
