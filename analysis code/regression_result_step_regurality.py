...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_regularity.csv'))['regularity'].std())
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        if dir_name[:8] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_regularity.csv'))
        #data_['arm'][data_['arm'] > 1] = 1 / data_['arm'][data_['arm'] > 1]

        original_data = data_
        original_variable = original_data['regularity']

        #normalization
        #data_ = remove_outliers_3(data_, 'reglarity')
        data_['regularity'] = (data_['regularity'] - data_['regularity'].min()) / (data_['regularity'].max() - data_['regularity'].min())
        ...
        pupil["rate_rolling"] = pupil['rate'].rolling(window=130).mean()


        data['stride_rolling'] = data['regularity'].rolling(window=30).mean()
        data['converttime'] = data['time'].apply(lambda x:convert_video(x))

        original_data['stride_rolling'] = original_data['regularity'].rolling(window=30).mean()
        original_data['converttime'] = original_data['time'].apply(lambda x: convert_video(x))

        #sstride = stride.iloc[::10,:]
        #pupil["rate_rolling"].plot(figsize=(20,10)))
        #stride['stride_rolling'].plot(figsize=(20,10))

        # dfmerge = pd.merge(pupil['rate_rolling'], stride['stride_rolling'])
        # dfmerge.plot(figsize=(20, 10))
        # plt.show()

        # pupil["time"] = pupil["time"].astype('int64')
...
plt.plot(after_1['rate_rolling'], after_1['trend'], color='#8CB6D5', label="low cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#8CB6D5', label="moderate cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#8CB6D5', label="high cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#8CB6D5', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#8CB6D5', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#8CB6D5', alpha=0.2)

plt.text(-0.075, y_axis_max - 0.01, 'r=-0.056, p<0.001*', fontsize=14)
plt.text(0, y_axis_max - 0.01, 'r=0.065, p=0.020*', fontsize=14)
plt.text(0.06, y_axis_max - 0.01, 'r=0.126, p<0.001*', fontsize=14)
...
plt.ylabel('Step regularity', fontsize = 20,fontname='Arial')
...
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_regularity_20.png')

plt.savefig(output_file_path, dpi=300, format='png', bbox_inches='tight')
plt.show()

stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'regularity_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'regularity_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'regularity_merge3.csv'),index=False)
