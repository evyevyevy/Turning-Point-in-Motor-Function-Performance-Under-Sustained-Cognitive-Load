...
def expected_mutual_info(X, Y, n_samples=100):

    actual_mi = mutual_info_score(X, Y)


    expected_mi = 0
    for _ in range(n_samples):
        random_X = np.random.choice(X, size=len(X), replace=False)
        expected_mi += mutual_info_score(random_X, Y)
    expected_mi /= n_samples

    return actual_mi, expected_mi
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_step_length.csv'))['step length'].std())
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        if dir_name[:8] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_step_length.csv'))
        # data['arm'][data['arm'] > 1] = 1 / data['arm'][data['arm'] > 1]

        original_data = data_
        original_variable = original_data['step length']

        # normalization
        data_['step length'] = (data_['step length'] - data_['step length'].min()) / (
                    data_['step length'].max() - data_['step length'].min())

        # data_['stride'] = (data_['stride'] - data_.iloc[0:5]['stride'].mean()) / data_.iloc[0:5]['stride'].mean()
        # data_ = remove_outliers_3(data_, 'stride')
        ...
        data['stride_rolling'] = data['step length'].rolling(window=15).mean()
        data['converttime'] = data['time'].apply(lambda x: convert_video(x))

        original_data['stride_rolling'] = original_data['step length'].rolling(window=15).mean()
        ...
        original_merge = pd.merge_asof(original_data, pupil, left_on='converttime', right_on="time",
                                       direction="nearest")
        ...
        if dir_name[-1] == '1':
            ...
            if len(original_merge) == 0: continue
            ...
            original_merge = original_merge[
                (original_merge['rate_rolling'] >= -0.005833185017907219) & (original_merge['rate_rolling'] <= 0.044140670424346704)]
            if len(original_merge) <= 1: continue
            correlation_coefficient, p_value = spearmanr(original_merge['rate_rolling'],
                                                         original_merge['stride_rolling'])
            ...
            original_merge = original_merge[
                (original_merge['rate_rolling'] >= 0.044140670424346704) & (original_merge['rate_rolling'] <= 0.11199983686890083)]
            if len(original_merge) <= 1: continue
            correlation_coefficient, p_value = spearmanr(original_merge['rate_rolling'],
                                                         original_merge['stride_rolling'])
...
# after_1 = stride_sort_1
# after_2 = stride_sort_2
# after_3 = stride_sort_3
# after_1 = stride_sort_1[(stride_sort_1['rate_rolling'] >= -0.1228) & (stride_sort_1['rate_rolling'] <= -0.013)]
...
plt.plot(after_1['rate_rolling'], after_1['trend'], color='#C7A5D4', label="low cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#C7A5D4', label="moderate cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#C7A5D4', label="high cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#C7A5D4', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#C7A5D4', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#C7A5D4', alpha=0.2)

plt.text(-0.075, y_axis_max - 0.01, 'r=0.041, p=0.074', fontsize=14)
plt.text(0, y_axis_max - 0.01, 'r=-0.074, p=0.006*', fontsize=14)
plt.text(0.06, y_axis_max - 0.01, 'r=0.060, p=0.005*', fontsize=14)

plt.text(-0.075, y_axis_max - 0.015, equation1, fontsize=14)
plt.text(0, y_axis_max - 0.015, equation2, fontsize=14)
plt.text(0.06, y_axis_max - 0.015, equation3, fontsize=14)


plt.xlabel('Pupil dilation ratio', fontsize = 20,fontname='Arial')
plt.ylabel('Step length', fontsize = 20,fontname='Arial')
...
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_step_length_40.png')
...
stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'step_length_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'step_length_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'step_length_merge3.csv'),index=False)
...
coefficients, stderrs, t_stats, p_values = calculate_p_values(stride_sort_1['rate_rolling'],
                                                              stride_sort_1['stride_rolling'])
print(coefficients, stderrs, t_stats, p_values)
coefficients, stderrs, t_stats, p_values = calculate_p_values(stride_sort_2['rate_rolling'],
                                                              stride_sort_2['stride_rolling'])
print(coefficients, stderrs, t_stats, p_values)
coefficients, stderrs, t_stats, p_values = calculate_p_values(stride_sort_3['rate_rolling'],
                                                              stride_sort_3['stride_rolling'])
print(coefficients, stderrs, t_stats, p_values)
