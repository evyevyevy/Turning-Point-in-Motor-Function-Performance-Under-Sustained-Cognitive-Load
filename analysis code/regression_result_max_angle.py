...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_max_angle.csv'))['max angle'].std())
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        if dir_name[:8] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_max_angle.csv'))
        #data_['arm'][data_['arm'] > 1] = 1 / data_['arm'][data_['arm'] > 1]

        original_data = data_
        original_variable = original_data['max angle']

        #normalization
        data_['max angle'] = (data_['max angle'] - data_['max angle'].min()) / (data_['max angle'].max() - data_['max angle'].min())
        ...
        data['stride_rolling'] = data['max angle'].rolling(window=2).mean()
        data['converttime'] = data['time'].apply(lambda x:convert_video(x))

        original_data['stride_rolling'] = original_data['max angle'].rolling(window=2).mean()
...
plt.plot(after_1['rate_rolling'], after_1['trend'], color='#DE752D', label="low cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#DE752D', label="moderate cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#DE752D', label="high cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#DE752D', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#DE752D', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#DE752D', alpha=0.2)

plt.text(-0.075, y_axis_max - 0.03, 'r=0.052, p=0.074', fontsize=14)
plt.text(0, y_axis_max - 0.03, 'r=0.016, p=0.780', fontsize=14)
plt.text(0.06, y_axis_max - 0.03, 'r=-0.039, p=0.383', fontsize=14)

plt.text(-0.075, y_axis_max - 0.05, equation1, fontsize=14)
plt.text(0, y_axis_max - 0.05, equation2, fontsize=14)
plt.text(0.06, y_axis_max - 0.05, equation3, fontsize=14)


plt.xlabel('Pupil dilation ratio', fontsize = 20,fontname='Arial')
plt.ylabel('Arm movement angle', fontsize = 20,fontname='Arial')
...
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_max_angle_5.png')
...
stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'max_angle_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'max_angle_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'max_angle_merge3.csv'),index=False)
