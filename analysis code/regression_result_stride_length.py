...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_stride.csv'))['stride'].std())
...
pearson_in_p_2 = []
pearson_in_p_3 = []
no_list = ['a_092306','a_080201','a_081002', 'b_092410','b_100106','a_092506','a_080212','a_100111']
x = 0
for home, dirs, files in os.walk(key_path):
    dirs.sort()
    for dir_name in dirs:
        if dir_name[:-3] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_stride.csv'))
        #data_['arm'][data_['arm'] > 1] = 1 / data_['arm'][data_['arm'] > 1]

        original_data = data_
        original_variable = original_data['stride']

        #normalization
        data_ = remove_outliers_3(data_, 'stride')
        data_['stride'] = (data_['stride'] - data_['stride'].min()) / (data_['stride'].max() - data_['stride'].min())
        ...
        original_data['stride_rolling'] = original_data['stride'].rolling(window=15).mean()
...
plt.plot(after_1['rate_rolling'], after_1['trend'], color='#6F4892', label="low cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#6F4892', label="moderate cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#6F4892', label="high cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#6F4892', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#6F4892', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#6F4892', alpha=0.2)

plt.text(-0.075, y_axis_max - 0.005, 'r=0.025, p=0.074', fontsize=14)
plt.text(0, y_axis_max - 0.005, 'r=-0.052, p=0.059', fontsize=14)
plt.text(0.06, y_axis_max - 0.005, 'r=0.036, p=0.106', fontsize=14)

plt.text(-0.075, y_axis_max - 0.01, equation1, fontsize=14)
plt.text(0, y_axis_max - 0.01, equation2, fontsize=14)
plt.text(0.06, y_axis_max - 0.01, equation3, fontsize=14)


plt.xlabel('Pupil dilation ratio', fontsize = 20,fontname='Arial')
plt.ylabel('Stride length', fontsize = 20,fontname='Arial')
...
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_stride_length_60.png')
...
# stride_sort_2 = stride_sort_2.dropna()
# stride_sort_3 = stride_sort_3.dropna()
stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'stride_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'stride_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'stride_merge3.csv'),index=False)
