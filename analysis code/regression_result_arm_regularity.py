...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        ...
        std_all.append(pd.read_csv(os.path.join(indexes, 'new_arm_rate.csv'))['arm'].std())
...
for home, dirs, files in os.walk(key_path):
    ...
    for dir_name in dirs:
        if dir_name[:8] in no_list:
            continue
        indexes = os.path.join(key_path, dir_name)
        data_ = pd.read_csv(os.path.join(indexes, 'new_arm_rate.csv'))
        #data_['arm'][data_['arm'] > 1] = 1 / data_['arm'][data_['arm'] > 1]

        original_data = data_
        original_variable = original_data['arm']

        #normalization
        data_['arm'] = (data_['arm'] - data_['arm'].min()) / (data_['arm'].max() - data_['arm'].min())
        ...
        pupil["rate_rolling"] = pupil['rate'].rolling(window=20).mean()


        data['stride_rolling'] = data['arm'].rolling(window=2).mean()
        data['converttime'] = data['time'].apply(lambda x:convert_video(x))

        original_data['stride_rolling'] = original_data['arm'].rolling(window=2).mean()
        ...
        if dir_name[-1] == '1' :
            ...
            original_merge = original_merge[
                (original_merge['rate_rolling'] >= -0.08957766246476156) & (
                            original_merge['rate_rolling'] <= -0.005833185017907219)]
            if len(original_merge) <= 1: continue
            correlation_coefficient, p_value = spearmanr(original_merge['rate_rolling'],
                                                        original_merge['stride_rolling'])
...
#plt.scatter(x=after_1['rate_rolling'], y=after_1['stride_rolling'], s=4, color='#4160A5')
#plt.scatter(x=after_2['rate_rolling'], y=after_2['stride_rolling'], s=4, color='#E28445')
#plt.scatter(x=after_3['rate_rolling'], y=after_3['stride_rolling'], s=4, color='#5EAB64')


#plt.scatter([], [], s=80, color='#4160A5', label='arm movement regularity in low CL')
#plt.scatter([], [], s=80, color='#E28445', label='arm movement regularity in moderate CL')
#plt.scatter([], [], s=80, color='#5EAB64', label='arm movement regularity in high CL')

plt.plot(after_1['rate_rolling'], after_1['trend'], color='#FDD712', label="low cognitive load", \
         linewidth=3, linestyle='-')
plt.plot(after_2['rate_rolling'], after_2['trend'], color='#FDD712', label="moderate cogntive load", \
         linewidth=3, linestyle='--')
plt.plot(after_3['rate_rolling'], after_3['trend'], color='#FDD712', label="high cognitive load", \
         linewidth=3, linestyle='-.')
plt.fill_between(after_1['rate_rolling'], lower_bound_1, upper_bound_1, color='#FDD712', alpha=0.2)
plt.fill_between(after_2['rate_rolling'], lower_bound_2, upper_bound_2, color='#FDD712', alpha=0.2)
plt.fill_between(after_3['rate_rolling'], lower_bound_3, upper_bound_3, color='#FDD712', alpha=0.2)

plt.text(-0.075, y_axis_max - 0.015, 'r=-0.011, p=0.698', fontsize=14)
plt.text(0, y_axis_max - 0.015, 'r=-0.054, p=0.347', fontsize=14)
plt.text(0.06, y_axis_max - 0.015, 'r=-0.026, p=0.566', fontsize=14)

plt.text(-0.075, y_axis_max - 0.03, equation1, fontsize=14)
plt.text(0, y_axis_max - 0.03, equation2, fontsize=14)
plt.text(0.06, y_axis_max - 0.03, equation3, fontsize=14)


plt.xlabel('Pupil dilation ratio', fontsize = 20,fontname='Arial')
plt.ylabel('Arm movement regularity', fontsize = 20,fontname='Arial')
...
output_file_path = os.path.join('G:/figure_nomal/figure-filter', 'pupil_arm_rate_5_!.png')

plt.savefig(output_file_path, dpi=800, format='png', bbox_inches='tight')
plt.show()



stride_sort_1.to_csv(os.path.join('G:/openpose_output', 'arm_rate_merge1.csv'),index=False)
stride_sort_2.to_csv(os.path.join('G:/openpose_output', 'arm_rate_merge2.csv'),index=False)
stride_sort_3.to_csv(os.path.join('G:/openpose_output', 'arm_rate_merge3.csv'),index=False)
